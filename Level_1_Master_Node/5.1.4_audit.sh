...
audit_rule() {
	...
	violations=$(kubectl get roles,clusterroles -A -o json 2>/dev/null | jq -r '.items[] | select(.rules[]? | select((.resources[]? == "*" or .resources[]? == "pods") and (.verbs[]? == "*" or .verbs[]? == "create" or .verbs[]? == "update" or .verbs[]? == "patch"))) | "\(.kind): \(.metadata.namespace):\(.metadata.name)"' | sort -u)
