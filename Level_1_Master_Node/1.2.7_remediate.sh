...
VALUE=$(echo "${CONFIG_REQUIRED_VALUE:-Node}" | tr -d '"')
MANIFEST_FILE="/etc/kubernetes/manifests/kube-apiserver.yaml"

echo "[INFO] Running Hardener: $HARDENER_SCRIPT"
echo "[INFO] Task: Setting --authorization-mode to $VALUE in $MANIFEST_FILE"
...
if [ $? -eq 0 ]; then
    echo "[FIXED] CIS 1.2.7: Successfully updated --authorization-mode"
    # Touch manifest to force kubelet reload of static pod
    touch "$MANIFEST_FILE"
    exit 0
else
    echo "[ERROR] Failed to update --authorization-mode"
    exit 1
fi
