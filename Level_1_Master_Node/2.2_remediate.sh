...
remediate_rule() {
	...
	## Edit the etcd pod specification file /etc/kubernetes/manifests/etcd.yaml on the master node and set the below parameter. --client-cert-auth="true"
	##
	## Command hint: Edit the etcd pod specification file /etc/kubernetes/manifests/etcd.yaml on the master node and set the below parameter. --client-cert-auth="true"
	##
	...
	if [ -e "$l_file" ]; then
		if grep -q "\--client-cert-auth" "$l_file"; then
			...
			a_output+=(" - Remediation applied: --client-cert-auth set to true in $l_file")
			return 0
		else
