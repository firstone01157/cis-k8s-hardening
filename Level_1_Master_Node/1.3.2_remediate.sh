#!/bin/bash
################################################################################
# CIS Kubernetes Benchmark 1.3.2 Remediation
# Ensure that the --profiling argument is set to false (Controller Manager)
# 
# Description:
#   Profiling should be disabled on the Controller Manager to prevent exposure
#   of sensitive performance and operational details of the controller components.
#
...
backup_manifest() {
    mkdir -p "$BACKUP_DIR"
    if cp "$CONTROLLER_MANIFEST" "$BACKUP_DIR/kube-controller-manager.yaml.bak"; then
        log_success "Backup created: $BACKUP_DIR/kube-controller-manager.yaml.bak"
        return 0
    else
...
main() {
    log_info "Starting CIS 1.3.2 Remediation: Controller Manager profiling disabled"
    log_info "Target manifest: $CONTROLLER_MANIFEST"
    
    # Check if manifest exists
    if [[ ! -f "$CONTROLLER_MANIFEST" ]]; then
        log_error "Controller Manager manifest not found: $CONTROLLER_MANIFEST"
        exit 1
    fi
    ...
    if grep -q "^\s*-\s*--profiling" "$CONTROLLER_MANIFEST"; then
        log_info "Flag already exists"
        
        # Check if it's set to false
        if grep -q "^\s*-\s*--profiling=false" "$CONTROLLER_MANIFEST"; then
            ...
            # Remove old flag
            sed -i "/--profiling=/d" "$CONTROLLER_MANIFEST"
        fi
    fi
    
    log_info "Adding --profiling=false to Controller Manager manifest..."
    
    # Add the flag after the kube-controller-manager line
    sed -i "/- kube-controller-manager/a\        - --profiling=false" "$CONTROLLER_MANIFEST"
    ...
    if grep -q "^\s*-\s*--profiling=false" "$CONTROLLER_MANIFEST"; then
        ...
        VERIFIED_LINE=$(grep "^\s*-\s*--profiling" "$CONTROLLER_MANIFEST")
        ...
        log_success "CIS 1.3.2 Remediation completed successfully"
        exit 0
    else
        log_error "Verification failed: --profiling=false not found in manifest"
        
        if cp "$BACKUP_DIR/kube-controller-manager.yaml.bak" "$CONTROLLER_MANIFEST"; then
