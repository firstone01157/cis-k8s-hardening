#!/bin/bash
################################################################################
# CIS Kubernetes Benchmark 1.2.30 Remediation
# Ensure that the --service-account-extend-token-expiration parameter is set to false
# 
# Description:
#   The --service-account-extend-token-expiration flag should be set to false
#   to prevent indefinite token validity. This ensures tokens eventually expire
#   and reduces the impact of token compromise.
#
...
main() {
    log_info "Starting CIS 1.2.30 Remediation: service-account-extend-token-expiration"
    ...
    log_info "Checking if --service-account-extend-token-expiration flag exists..."
    
    if grep -q "service-account-extend-token-expiration" "$API_SERVER_MANIFEST"; then
        log_info "Flag already exists"
        
        # Check if it's set to false
        if grep -q "service-account-extend-token-expiration=false" "$API_SERVER_MANIFEST"; then
            ...
            # Remove old flag
            sed -i "/service-account-extend-token-expiration=/d" "$API_SERVER_MANIFEST"
        fi
    fi
    
    log_info "Adding --service-account-extend-token-expiration=false to manifest..."
    
    # Add the flag after the kube-apiserver line
    sed -i "/- kube-apiserver/a\        - --service-account-extend-token-expiration=false" "$API_SERVER_MANIFEST"
    
    # Verification step
    log_info "Verifying the change..."
    
    if grep -q "service-account-extend-token-expiration=false" "$API_SERVER_MANIFEST"; then
        log_success "Verification passed: --service-account-extend-token-expiration=false found"
        
        # Display the verified line
        VERIFIED_LINE=$(grep "service-account-extend-token-expiration" "$API_SERVER_MANIFEST")
        ...
        log_success "CIS 1.2.30 Remediation completed successfully"
        exit 0
    else
        log_error "Verification failed: flag not found in manifest"
