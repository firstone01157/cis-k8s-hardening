...
# Ensure that the --service-account-extend-token-expiration argument is set to false
# Level: 1 - Master Node
...
echo "[INFO] Task: Setting --service-account-extend-token-expiration to $VALUE in $MANIFEST_FILE"
...
if [ $? -eq 0 ]; then
    echo "[FIXED] CIS 1.2.30: Successfully updated --service-account-extend-token-expiration"
    # Touch manifest to force kubelet reload of static pod
    touch "$MANIFEST_FILE"
    exit 0
else
    echo "[ERROR] Failed to update --service-account-extend-token-expiration"
    exit 1
fi
