...
ROTATE_SERVER_CERTS=${CONFIG_ROTATE_SERVER_CERTIFICATES:-true}
...
echo "[INFO] Setting rotateServerCertificates to $ROTATE_SERVER_CERTS"

# Call Python helper
if python3 "$HELPER_SCRIPT" \
    --config "$KUBELET_CONFIG" \
    --key "rotateServerCertificates" \
    --value "$ROTATE_SERVER_CERTS"; then
    echo "[INFO] Restarting kubelet..."
    if systemctl restart kubelet 2>&1; then
        sleep 2
        if systemctl is-active --quiet kubelet; then
            echo "[PASS] 4.2.11 remediation complete"
            exit 0
        else
